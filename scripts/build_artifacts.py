...
from bwar.paper_jcgs.matched_reference_drift import make_figure
...
def main() -> None:
    ...
    summary = pd.read_csv(
        args.results_root / "s2_reference_adaptation" / "summary.csv"
    )
    make_figure(
        summary,
        figures / "s2_matched_start_loss",
        reps=int(summary["n_replications"].max()),
    )
